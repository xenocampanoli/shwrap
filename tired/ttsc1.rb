...
def validateTestEx(tE)
	return tE if tE == true
	return tE if tE == "sudo"
	return tE if tE == false
...
def validateTestHost(tH)
	...
	tH.split('.').each do |qw|
		unless validQuadWord(qw)
			raise ArgumentError, "Invalid quadword '#{qw}' in '#{tH}'."
		end
		qwc += 1
	end
	unless qwc == 4
		raise ArgumentError, "Invalid quadword count of '#{qwc}' in '#{tH}'."
	end
	return tH
end
