...
unless ARGV.length == 2
	puts "USAGE:  ruby user_test_shell_cmds.rb <TestHost> <UserName>"
	puts "  <TestHost> is for now strictly the IP address of the host."
	puts "  <UserName> is the user under test, and the administrative user."
	exit 1
end
...
describe "unix commands work fine." do
	...
	describe "test grep command" do
		it "should filter text for a string." do
			to1 = Shwrap::Tester.new(AccO,"ps auxw | grep e | grep -v root | grep -c #{TestUser}",[],TuEx,16)
	...
	describe "test ls command" do
		...
		it "should write to standard out." do
			...
			to = Shwrap::Tester.new(AccO,"ls -al #{TmpDir} >#{tlsf}",[tailto],TuEx,16)
	...
	describe "test printf command" do
		it "should format data and print it." do
			to = Shwrap::Tester.new(AccO,"printf \"%x\" \$(curl http://distrowatch.org | wc -c)",[],TuEx,16)
	...
	describe "test tr command" do
		it "should filter a set of characters in a file." do
			to = Shwrap::Tester.new(AccO,"curl http://www.eskimo.com 2>/dev/null | tr \"</>\n\" \".\"",[],TuEx,16)
			to.Result.split(/\n/).length.must_equal 1
		...
		it "should delete a set of characters in a file." do
			to = Shwrap::Tester.new(AccO,"curl http://www.eskimo.com 2>/dev/null | tr -d \"</>\n\"",[],TuEx,16)
			to.Result.split(/\n/).length.must_equal 1
			to.Result.wont_match /[<>\/\n]/
		end 
	...
	describe "test uname command" do
		it "should show system naming information." do
			...
			to5 = Shwrap::Tester.new(AccO,"uname -p",[],TuEx,16)
			to5.Result.must_match /\d+/
			to6 = Shwrap::Tester.new(AccO,"uname -i",[],TuEx,16)
			to6.Result.must_match /\d+/
	...
	describe "test wget command" do
		it "should write a file with contents of the markup for an HTTP URL." do
			to = Shwrap::Tester.new(AccO,"wget -o /tmp/wgettest.log -O /tmp/wgettest.html http://www.google.com;cat /tmp/wgettest.html",[],TuEx,16)
			to.Result.must_match /html/i
			to.Result.must_match /head/i
			to.Result.must_match /body/i
		end 
	end
